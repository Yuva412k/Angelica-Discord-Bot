...
class Player extends EventEmitter{
    ...
    skip (message) {
        ...
        const currentTrack = server.playing
        // End the dispatcher
        server.voiceConnection.dispatcher.end()
        // Return the queue
        return server
    ...
    shuffle (message) {
        ...
        const currentTrack = server.tracks.shift()
        server.tracks = server.tracks.sort(() => Math.random() - 0.5)
        server.tracks.unshift(currentTrack)
        // Return the queue
        return server
    }
    ...
    remove (message, track) {
        ...
        if (!serverQueue) return this.emit('error', 'NotPlaying', message)
            ...
            collector.on('end', async (collected,reason) => {
                ...
                if(selectedSongs.length !== 0){
                    if (this.isPlaying(message)) {
                        ...
                        const track = selectedSongs.shift()
